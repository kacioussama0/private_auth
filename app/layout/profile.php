...
<div class="w-full max-w-sm  border border-gray-700 rounded-lg shadow">
        ...
        <form method="post">
            <div id="dropdown" class="z-10 hidden text-base list-none bg-[#09090d] rounded-lg shadow w-44">
                <ul class="py-2" aria-labelledby="dropdownButton">
                    <li>
                        <a href="?page=account-settings" class="block px-4 py-2 text-sm text-white hover:bg-purple-700">Account Settings</a>
                    </li>
                    <li>
                        <a href="?page=account-logs" class="block px-4 py-2 text-sm text-white hover:bg-purple-700">Account
                            Logs</a>
                    </li>
                    <li>
                        <a href="?page=logout" class="block px-4 py-2 text-sm text-white hover:bg-purple-700">Log Out</a>
                    </li>
                </ul>
            </div>
        </form>
    ...
    <div class="flex flex-col items-center pb-4">
        ...
        <?php } else { ?>
            <h5 class="mb-1 text-xl font-medium text-purple-700"><?= $_SESSION["username"]; ?></h5>
        <?php } ?>

        <?php
        $cssClasses = " bg-gradient-to-r to-purple-600 from-violet-400 text-xs font-black mr-2 px-1.5 py-0.5 rounded border mb-2 mt-2";

        if ($role == "seller") {
            // If the role is "seller", add these classes
            $cssClasses .= " border-purple-400";
